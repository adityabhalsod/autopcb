...
from .design_engine import Component, ICDesign, Net

log = logging.getLogger("autopcb.drc")
...
class DRCReport:
    ...
    def from_dict(cls, raw: dict) -> "DRCReport":
        rep = cls(
            violations=[
                DRCViolation(**v) for v in raw.get("violations", []) if isinstance(v, dict)
            ],
        )
        rep.recount()
    ...
    def recount(self) -> None:
        ...
        self.summary = f"{self.fail_count} FAIL · {self.warn_count} WARN · {self.pass_count} PASS"
...
class DRCEngine:
    ...
    def register_rule(
        self, rule_id: str, callback, severity: str = SEV_WARN, description: str = ""
    ) -> None:
    ...
    def run_deterministic(self, design: ICDesign) -> DRCReport:
        ...
        if ic_type in ("analog", "mixed", "power"):
            ...
            has_bypass = any(
                c.type.upper() == "CAP" and any(p.upper() in ("VDD", "GND") for p in c.pins)
        ...
        if not rep.violations:
            rep.violations.append(
                DRCViolation(
                    ...
                    severity=SEV_PASS,
                    component_ref="*",
                    message="Deterministic DRC passed",
                    suggested_fix="",
                )
            )
